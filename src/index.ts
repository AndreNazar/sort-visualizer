...
let isSorted = true // Сначала он сортирован, так как там одни нули
...
function drawBars(indexpivot: number, indexleft: number, indexright: number) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (indexpivot != -1 && indexleft != -1 && indexright != -1) ctx.fillStyle = i === indexpivot ? "#ed5761" : i === indexleft || i === indexright ? "#404b62" : "#161515"
...
play.addEventListener("click", async () => {
  if (!buttonPlayed && !buttonShuffled && !isSorted) {
    buttonPlayed = true
